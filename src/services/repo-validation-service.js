...
export async function validateRepositoryPath(inputPath) {
  ...
  const isBareRepository = await gitBoolean(normalizedPath, ["rev-parse", "--is-bare-repository"]);
  ...
  const insideWorkTree = await gitBoolean(normalizedPath, ["rev-parse", "--is-inside-work-tree"]);
