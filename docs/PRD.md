...
- 统一产品定义，不再把 `docs/PRD.md` 和 `docs/v1.1/` 当成两个时代的并行 PRD
- 将已经进入代码和交互面的能力正式纳入产品范围
- 为后续 phase 文档、迁移文档、API 示例和实现工作提供顶层约束
...
EAT 是一个运行在本机上的 Web 工作台，让操作者围绕本地 Git 仓库，以 Lead Agent 为唯一主要编排入口，在独立分支、独立 worktree、独立 Docker 沙箱中调度多个 Worker Agent，并在浏览器中监督澄清、计划、执行、审查、集成、预览和收口全过程。
...
- 用 Git 分支、worktree 和 Docker 维持执行隔离
...
允许存在 `HOST` sandbox 能力声明，但当前产品对 Worker 的默认和主路径要求仍然是 `DOCKER`。
...
6. 系统物化 `SubTask`，建立 team 视图并在 Docker 容器中启动 Worker
...
#### FR-EX-03 Docker Sandbox

每个 Worker session 必须：
...
- UI：静态 HTML / CSS / ES modules
- Server：Node.js `node:http`
...
一个以 Lead 为核心、以 role-aware DAG 为执行载体、以 Docker sandbox 为安全边界、以 mailbox / board / integration / preview 为监督界面的本地 Web 编排工作台。

后续所有 phase、schema、API、UI 和实现工作，都应服从这个定义。
