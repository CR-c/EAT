...
当前正式默认实现仍然是 `DOCKER`。  
允许存在 `HOST` sandbox 能力声明；当显式设置 `EAT_ENABLE_TRUSTED_HOST_BACKEND=1` 时，系统会暴露 reduced-isolation 的 host backend 作为开发态降级路径。无论使用哪种实现，只有在 execution backend ready 时才允许批准执行。无 execution backend 时，系统允许 Lead-only 模式完成任务创建、需求澄清和计划生成。
