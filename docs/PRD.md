...
当前正式默认实现仍然是 `DOCKER`。  
允许存在 `HOST` sandbox 能力声明；当显式设置 `EAT_ENABLE_TRUSTED_HOST_BACKEND=1` 时，系统会暴露 reduced-isolation 的 host backend 作为开发态降级路径。默认情况下，host backend 只允许在 orchestrator 管理的 worktree 根下执行；若需扩大范围，必须再通过 `EAT_TRUSTED_HOST_ALLOWED_ROOTS` 显式声明。无论使用哪种实现，只有在 execution backend ready 时才允许批准执行。无 execution backend 时，系统允许 Lead-only 模式完成任务创建、需求澄清和计划生成。
...
6. 系统先校验 execution backend readiness；通过后物化 `SubTask`，建立 team 视图并在当前默认 backend 中启动 Worker（正式默认仍为 Docker；若显式开启 `EAT_ENABLE_TRUSTED_HOST_BACKEND=1` 且其成为 default backend，则会走 reduced-isolation 的 host 路径）
...
- 运行在独立 Docker 容器中
- 允许显式启用 `HOST` reduced-isolation backend 作为受信任本机开发态降级路径，但它不能伪装成与 Docker 同级安全
- 将 worktree 以读写方式挂载
- 将附件按需只读挂载
- 不默认暴露宿主关键目录
