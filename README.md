...
- 一个任务对应一条任务主线分支；子任务在各自的分支、worktree 和 Docker 沙箱中执行，成功结果再汇回任务主线。
...
当前 stock runtime 会把 Docker worker sandbox 视为 Agent 健康检查的一部分。也就是说，在默认配置下：

- 没有本地 `eat/worker-base:latest` 镜像时，`/api/agents/health` 会报告 Worker sandbox 不可用
- 创建任务时如果所选 Lead health 失败，`/api/tasks` 和 `/api/guided-tasks` 会被拒绝
- 因此本地试跑任务主流程前，应先构建 Worker 镜像
