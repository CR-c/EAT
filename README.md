...
- 前端已新增 `web/src/lib/platform.ts` 作为平台适配层；未来桌面壳可通过 `window.__EAT_PLATFORM__` 注入 `apiBaseUrl`
...
- 可显式开启 reduced-isolation 的 `host` execution backend（`EAT_ENABLE_TRUSTED_HOST_BACKEND=1`），仅用于受信任本机开发环境
- host backend 默认只允许在 orchestrator 管理的 `.eat-worktrees` 根下执行；可通过 `EAT_TRUSTED_HOST_ALLOWED_ROOTS` 显式追加允许根路径
...
- 任务在创建时会固化 `workerBackendKind`（默认取当前 default backend）；`executionProfile` 当前支持 `default` / `isolated` / `internet` / `host-network` / `web-preview` / `web-preview-host`，并会映射到 worker 的网络档位；其中 `web-preview` / `web-preview-host` 还会注入预览端口环境变量。
- 任务附件在 worker 执行时会以只读挂载形式提供给执行环境，供 agent 读取参考。
