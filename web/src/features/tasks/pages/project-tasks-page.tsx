      ...
      <div className="relative z-10 flex h-full flex-col p-8">
        <div className={cn("mb-6 flex items-end justify-between border-b pb-4", theme.sidebarBorder)}>
          <div>
            <div className={cn("mb-1 font-mono text-sm tracking-[0.2em]", theme.pageSub)}>
              {t("task.subtitle")} {t("common.subtitleSlash")}
            </div>
            <h2 className={cn("font-mono text-3xl font-black tracking-widest", theme.pageTitle)}>{t("task.title")}</h2>
          </div>
          <Link
        ...
        <div className="mb-6 flex flex-col items-center justify-between space-y-4 sm:flex-row sm:space-y-0">
          <div className={cn("flex space-x-1 border-b font-mono text-sm", theme.sidebarBorder)}>
            {[
              { key: "all", label: t("task.tabs.all") },
              { key: "active", label: t("task.tabs.active") },
              { key: "archived", label: t("task.tabs.archived") },
            ].map((tab) => (
              <button
                key={tab.key}
                className={cn(
                  ...
                  <div className={cn("relative z-10 mt-4 flex items-center justify-between rounded-sm border p-2.5 font-mono text-xs", theme.pathBg)}>
                    <div className="flex items-center overflow-hidden">
                      ...
                      <span className={cn("truncate font-bold", isRei ? "text-cyan-600" : "text-green-400")} title={task.taskBranchName ?? ""}>
                        {task.taskBranchName ?? t("common.autoGenerated")}
                      </span>
                    </div>
                  </div>
        ...
        <div className={cn("mt-4 font-mono text-xs", theme.cardSub)}>
          {project.data?.project.path ?? t("common.syncingProjectContext")}
        </div>
      </div>
    </>
  )
